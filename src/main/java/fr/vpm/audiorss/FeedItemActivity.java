...
public class FeedItemActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (item != null) {
      ...
      SimpleDateFormat datePrinter = new SimpleDateFormat(
          "EEEE, dd MMMM yyyy - HH:mm");
      SimpleDateFormat dateParser = new SimpleDateFormat(
          RSSChannel.DATE_PATTERN, Locale.US);
      try {
  ...
  private boolean isMediaDownloaded() {
    /*
     * if (item.getMediaUri() == null) { long fileId = item.getMediaId(); Uri
     * mediaUri = ItemParser.retrieveUri(fileId, this); if (mediaUri != null) {
    ...
    BroadcastReceiver downloadFinished = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        Log.d("BReceiver", "The status for " + id + " is " + status
            + ". It is located at " + localUri);
        item.setMediaUri(localUri);
      }
    };
